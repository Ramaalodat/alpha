...
final TextEditingController customCategoryController =
  ...
  Future<void> loadExpenses() async {
    ...
    if (decodedData is! List) {
      throw const FormatException(
        'Saved expenses data is invalid',
      );
    }

    final loadedExpenses = decodedData
        .whereType<Map>()
        .map(
        ...
        .where(
          (expense) => expense.id.trim().isNotEmpty,
        )
        .toList();
  ...
  Future<bool> _saveExpenses() async {
    _isSaving = true;
    _errorMessage = null;
    ...
    try {
      ...
      if (!saved) {
        throw Exception(
          'Could not save expenses locally',
        );
      }
