...
export const requireOnboarding = async (
  ...
  if (!request.user.isOnboarded) {
    // Double check DB in case the token is stale (e.g. just completed onboarding)
    let isOnboardedInDb = false;
    try {
      ...
      if (dbUser && dbUser.isOnboarded) {
        isOnboardedInDb = true;
      } else if (!dbUser) {
        // Fallback to dev store if DB fails
        const devUser = await devStore.findUserById(request.user.userId);
        if (devUser && devUser.isOnboarded) isOnboardedInDb = true;
      }
    } catch (err) {
      // If DB error, try dev store
      const devUser = await devStore.findUserById(request.user.userId);
      if (devUser && devUser.isOnboarded) isOnboardedInDb = true;
    }

    if (isOnboardedInDb) {
      request.user.isOnboarded = true;
    } else {
