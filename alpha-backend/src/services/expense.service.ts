  ...
  async seedDefaultCategories(): Promise<void> {
    ...
    try {
      ...
      if (existingCategories === 0) {
        ...
        logger.info('Default expense categories seeded (Prisma)');
      }
    } catch (error: any) {
      const message = error instanceof Error ? error.message : String(error);
      if (/Can't reach database server|database server|ECONNREFUSED|P1001|P1000|Connection refused/i.test(message)) {
        logger.warn('Prisma unavailable, seeding dev store categories');
      } else {
        throw error;
      }
    }

    // Always seed dev store categories
    const existingDevCat = await devStore.findCategory((c) => c.isDefault);
    if (!existingDevCat) {
      for (const [index, category] of DEFAULT_EXPENSE_CATEGORIES.entries()) {
        await devStore.upsertCategory({
          name: category.name,
          nameAr: category.name,
          icon: category.icon,
          color: category.color,
      ...
      logger.info('Default expense categories seeded (dev store)');
    }
  }
