...
export class UserService {
    ...
    return this.useDevFallback(
      async () => {
        ...
        const newProfile = await prisma.userProfile.create({
          data: {
            userId,
            monthlyIncome: profileData.monthlyIncome ?? currentProfile?.monthlyIncome ?? 0,
            basicExpenses: profileData.basicExpenses ?? currentProfile?.basicExpenses ?? 0,
            financialGoal: profileData.financialGoal ?? currentProfile?.financialGoal,
            primarySpendingCategory: profileData.primarySpendingCategory ?? currentProfile?.primarySpendingCategory ?? 'غير محدد',
            ...
            familySize: profileData.familySize ?? currentProfile?.familySize ?? 1,
            hasEmergencyFund: profileData.hasEmergencyFund ?? currentProfile?.hasEmergencyFund ?? false,
            riskTolerance: profileData.riskTolerance ?? currentProfile?.riskTolerance,
            version: currentProfile ? currentProfile.version + 1 : 1,
            isCurrent: true,
            changeReason,
            createdBy: userId,
          },
      ...
      async () => {
        ...
        // Mark old profile as no longer current (preserves history)
        if (currentProfile) {
          await devStore.updateProfile(currentProfile.id, { isCurrent: false });
        }
  ...
  /**
   * Update user demographics (gender, marital status, etc.)
   */
  async updateDemographics(params: UpdateDemographicsParams): Promise<User> {
    const { userId, gender, maritalStatus, isHeadOfHousehold, isStudent } = params;

    return this.useDevFallback(
      async () => {
        const updateData: any = { gender: gender as any };
        if (maritalStatus) updateData.maritalStatus = maritalStatus as any;
        if (isHeadOfHousehold !== undefined) updateData.isHeadOfHousehold = isHeadOfHousehold;
