...
class FinancialProvider extends ChangeNotifier {
  ...
  final householdIncomeController = TextEditingController();

  final customMainGoalController = TextEditingController();
  ...
  void setHouseholdIncome(String value) {
    householdIncome = double.tryParse(value.replaceAll(",", "."));
  ...
  double get balance {
    return (householdIncome ?? 0) - (totalExpenses + (savingTarget ?? 0));
  }
  ...
  double get pageProgress {
    ...
    final selectedIncome = incomeSources.where((e) => e.selected).toList();

    if (selectedIncome.isNotEmpty &&
        selectedIncome.every((e) => e.amount > 0)) {
      completed++;
    }

    final selectedFixed = fixedExpenses.where((e) => e.selected).toList();

    if (selectedFixed.isNotEmpty && selectedFixed.every((e) => e.amount > 0)) {
      completed++;
    }

    final selectedFlexible = flexibleExpenses.where((e) => e.selected).toList();

    if (selectedFlexible.isNotEmpty &&
        selectedFlexible.every((e) => e.amount > 0)) {
      completed++;
    }
  ...
  Map<String, dynamic> get data => {
        "money_relationship": moneyRelationship,
        "saving_target": savingTarget,
        "main_goal": mainGoal == 'Other' ? customMainGoalController.text : mainGoal,
  ...
  void dispose() {
    ...
    customMainGoalController.dispose();
