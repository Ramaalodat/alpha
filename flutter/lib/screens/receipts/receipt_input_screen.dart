...
class _ReceiptInputScreenState extends State<ReceiptInputScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasVoiceText = receiptProvider.voiceText.trim().isNotEmpty;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          onPressed: () async {
            await context.read<ReceiptProvider>().cancelVoiceInput();
              ...
              child: Column(
                children: [
                  ...
                  _VoiceLanguageSelector(
                    selectedLocale: receiptProvider.selectedVoiceLocale,
                    enabled: !receiptProvider.isListening,
                    isDark: isDark,
                    onChanged: (locale) {
                      context.read<ReceiptProvider>().changeVoiceLocale(locale);
                    },
                  ),
                  if (hasVoiceText) ...[
                    const SizedBox(height: 14),
                    _EditableVoiceTextCard(
                      text: receiptProvider.voiceText,
                      isListening: receiptProvider.isListening,
                      isDark: isDark,
                      onChanged: (value) {
                        context.read<ReceiptProvider>().updateVoiceText(value);
                      },
                    ),
                  ],
                  if (receiptProvider.errorMessage != null) ...[
                    const SizedBox(height: 14),
                    _ErrorCard(
                      message: receiptProvider.errorMessage!,
                      onClose: () {
                        context.read<ReceiptProvider>().clearError();
                      },
                    ),
                  ],
                  ...
                  _InputControls(
                    ...
                    onCameraPressed: _captureReceipt,
                  ...
                  Text(
                    ...
                    textAlign: TextAlign.center,
                    style: GoogleFonts.ibmPlexSansArabic(
                      color: receiptProvider.isListening
                          ? const Color(
                              0xFF34D399,
                            )
                          : isDark
                  ...
                  if (hasVoiceText && !receiptProvider.isListening) ...[
                    const SizedBox(height: 18),
                    _AnalyzeVoiceButton(
                      isLoading: receiptProvider.isProcessing,
                      onPressed: _analyzeVoiceText,
                    ),
                  ],
                ],
              ),
            ),
            if (receiptProvider.isProcessing || _isTakingPicture)
              _ProcessingOverlay(
                isDark: isDark,
              ),
          ],
  ...
  Widget _buildCameraScanner(
    ...
    return AspectRatio(
      ...
      child: ClipRRect(
        ...
        child: Container(
          color: isDark ? const Color(0xFF071512) : const Color(0xFFE8F2EE),
          child: Stack(
            fit: StackFit.expand,
            children: [
  ...
  Widget _buildCameraPreview(
    ...
    if (controller == null || cameraFuture == null) {
      return const Center(
  ...
  Future<void> _captureReceipt() async {
    ...
    try {
      ...
      final result = await context.read<ReceiptProvider>().processCapturedImage(
            image.path,
          );
      ...
      _showMessage(
        error.description ?? 'Could not capture receipt',
      );
    } catch (error) {
      if (!mounted) return;
  ...
  Future<void> _pickFromGallery() async {
    final result =
        await context.read<ReceiptProvider>().pickReceiptFromGallery();
  ...
  Future<void> _analyzeVoiceText() async {
    ...
    final result = await context.read<ReceiptProvider>().parseEditedVoiceText();
  ...
  Future<void> _toggleFlash() async {
    ...
    try {
      ...
      _showMessage(
        error.description ?? 'Flash is unavailable',
      );
    }
  }
...
class _VoiceLanguageSelector extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final value = selectedLocale == 'en_US' ? 'en_US' : 'ar_JO';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isDark ? const Color(0xFF142723) : Colors.white,
...
class _EditableVoiceTextCard extends StatefulWidget {
  final String text;
  ...
  @override
  State<_EditableVoiceTextCard> createState() => _EditableVoiceTextCardState();
}

class _EditableVoiceTextCardState extends State<_EditableVoiceTextCard> {
  ...
  void didUpdateWidget(
    covariant _EditableVoiceTextCard oldWidget,
  ) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: widget.isDark ? const Color(0xFF142723) : Colors.white,
        borderRadius: BorderRadius.circular(17),
        border: Border.all(
          color: widget.isListening
              ? const Color(0xFF34D399).withOpacity(0.40)
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Review and edit the recognized text',
...
class _AnalyzeVoiceButton extends StatelessWidget {
...
class _InputControls extends StatelessWidget {
  final bool isDark;
  final bool isListening;
